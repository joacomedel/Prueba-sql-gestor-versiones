CREATE OR REPLACE FUNCTION public.far_buscarinfomedicamentosteniendoclave(character varying)
...
SELECT  a.idarticulo,a.idcentroarticulo  
  ...
  , CASE WHEN p.tabla ilike 'far_preciocompra' 
            THEN (case when nullvalue(a.adescripcion) then concat(mnombre , ' ' , mpresentacion , ' $ ' , vm.vmimporte::text, ' ' ,me.mtroquel::text ) ELSE concat(a.adescripcion , ' $ ' , pc.preciocompra::text) END )
            ELSE (case when nullvalue(a.adescripcion) then concat(mnombre , ' ' , mpresentacion , ' $ ' , vm.vmimporte::text, ' ' ,me.mtroquel::text ) ELSE concat(a.adescripcion , ' $ ' , pa.pavalor::text) END)  
  ...
  */
  ...
  , CASE WHEN p.tabla ilike 'far_preciocompra'
              THEN ( case when nullvalue(a.adescripcion) then concat(mnombre , ' ', mpresentacion, ' $ ' , pc.preciocompra::text , ' ' ,me.mtroquel::text) ELSE concat(a.adescripcion , ' $ ', pc.preciocompra::text) END  )
              ELSE ( case when nullvalue(a.adescripcion) then concat(mnombre , ' ', mpresentacion, ' $ ' , pa.pavalor::text , ' ' ,me.mtroquel::text) ELSE concat(a.adescripcion , ' $ ', pa.pavalor::text) END  )
              END as articulodetalle
  ,far_darcantidadarticulostock(a.idarticulo,a.idcentroarticulo)::bigint as lstock  
  ,me.mtroquel as troquel
  ,me.mpresentacion as presentacion
