CREATE OR REPLACE FUNCTION public.far_asentarfacturaventa_v3()
    ...
    -- SELECT INTO resp far_cambiarestadoordenventa(nroorden,centro,3) FROM temporden;
     ...
     IF (tipoorden=44) THEN --   Se esta facturando una orden de farmacia
                 ...
                 WHILE FOUND LOOP
                       ...
                       END IF;
                       IF (unitemfact.oviidiva = 1 ) THEN
                           sumaexento = sumaexento + round(CAST((unitemfact.ovipreciolista * unitemfact.ovicantidad) AS numeric),2);
                       ELSE
                           sumaconiva = sumaconiva + round(CAST((unitemfact.ovipreciolista * unitemfact.ovicantidad) AS numeric),2);
                       END IF;
		          ...
		          IF(importedescuento<>0)THEN
                           ...
                           END IF;
		                    ...
		                    INSERT INTO itemfacturaventa(tipocomprobante,nrosucursal,tipofactura,nrofactura,
                                         idconcepto,cantidad,importe,descripcion,idiva)
			                VALUES(elem.tipocomprobante,elem.nrosucursal,elem.tipofactura,elem.sgtenumero,
                                         50840,1,round(CAST (importedescuento AS numeric),2)*-1,'Descuentos y Bonificaciones',elidiva);
		          END IF;
