CREATE OR REPLACE FUNCTION public.asentarvalorizada(tipoc integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
DECLARE
...
--  Llama a asentarOrden()
  ...
  --  crea la tabla temporal TTFacturasGeneradas
...
-- Asienta en ItemsValorizada
    ...
    WHILE found LOOP
			...
			INSERT INTO item (cantidad,importe,idnomenclador,idcapitulo,idsubcapitulo,idpractica)
			       VALUES (unitem.cantidad,unitem.importe,nomenclador,capitulo,subcapitulo,practicaitem);			
			identitem = currval('"public"."item_iditem_seq"');			
			INSERT INTO itemvalorizada
