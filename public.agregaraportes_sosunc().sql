CREATE OR REPLACE FUNCTION public.agregaraportes_sosunc()
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$/*
...
/*Dani agrego el 23082021 para que agregue a los pasantes, a los cuales no se les liquidada el concepto 202*/                     
                     natural join 
                         (select idcategoria,idpersona from
                                ca.categoriaempleado
                                  where  (idcategoriatipo=1)and(nullvalue(cefechafin) 
                                  or concat(date_part('year', current_date - 30),'-',lpad(date_part('month', current_date - 30),2,'0') ,'-01')<=cefechafin)
                         )as datoscategoria
                     NATURAL JOIN ca.persona JOIN ( SELECT nrodoc,tipodoc,MAX(idcargo)as idcargo,legajosiu		
                      ...
                      WHERE  limes=date_part('month', current_date -30) and lianio=date_part('year', current_date - 30)  
...
/*Dani agrego el 23082021 para que agregue a los pasantes, a los cuales no se les liquidada el concepto 202*/                     
                     natural join 
                         (select idcategoria,idpersona,cefechainicio from
                                ca.categoriaempleado
                                  where  (idcategoriatipo=1)and(nullvalue(cefechafin) 
                                   or concat(date_part('year', current_date - 30),'-',lpad(date_part('month', current_date - 30),2,'0') ,'-01')<=cefechafin)
                         )as datoscategoria
                     ...
                     NATURAL JOIN ca.persona JOIN ( SELECT nrodoc,tipodoc,MAX(idcargo)as idcargo,legajosiu,fechainilab,fechafinlab	
                      ...
                      WHERE 
/*Dani comento el 23082021 porq deja afuera las persona sq si tienen una fechafin en el campo cefechafin*/
 
 limes=date_part('month', current_date -30) and 
                      lianio=date_part('year', current_date - 30)    
