...
BEGIN
    ...
    /*Lo Agrego MaLaPi 14-12-2007 para el caso de la emision de recetarios gratuitos*/
              IF nullvalue(dato.amuc)
                 AND nullvalue(dato.efectivo)
                 AND nullvalue(dato.cuentacorriente)
                 AND nullvalue(dato.debito)
                 AND nullvalue(dato.credito)
                 AND nullvalue(dato.sosunc) THEN
                 ...
                 END IF;
              END IF;
            if not nullvalue(dato.amuc) then
              INSERT INTO importesrecibo
                        VALUES (resp,1,dato.amuc,dato.centro);
              end if;

              if not nullvalue(dato.efectivo) then
              INSERT INTO importesrecibo
                     VALUES (resp,2,dato.efectivo,dato.centro);
              end if;

              if not nullvalue(dato.cuentacorriente) then
                 INSERT INTO importesrecibo
                        VALUES (resp,3,dato.cuentacorriente,dato.centro);
              end if;

              if not nullvalue(dato.debito) then
                 INSERT INTO importesrecibo
                        VALUES (resp,4,dato.debito,dato.centro);
              end if;
              if not nullvalue(dato.credito) then
                 INSERT INTO importesrecibo
                        VALUES (resp,5,dato.credito,dato.centro);
              end if;
              if not nullvalue(dato.sosunc) then
                 INSERT INTO importesrecibo
