CREATE OR REPLACE FUNCTION public.conciliacionbancaria_auto_mp_pagoordenpagocontable(character varying)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$/****/
DECLARE
    ...
    rusuario RECORD;
    cmovimientossiges CURSOR FOR SELECT * FROM  temp_movsiges WHERE tablacomp='pagoordenpagocontable'; -- temporal con los movimientos de siges que se desean conciliar
    auxcomprob varchar;
    sqllaclave varchar;
    sql  varchar;
...
BEGIN
    ...
    WHILE FOUND LOOP
        ...
        VALUES (rmovbancomp.bmusuario,rmovbancomp.bmfecha,rmovbancomp.idbancamovimiento,rmovbancomp.idcentroconciliacionbancaria,rmovbancomp.idconciliacionbancaria,rmovbancomp.bmconcepto,rmovbancomp.bmcodigo,rmovbancomp.bmsaldo,rmovbancomp.bmingreso,rmovbancomp.bmdebito,rmovbancomp.bmnrocomprobante,rmovbancomp.bmcredito );
            ...
            WHERE (POSITION(auxcomprob IN detalle) > 0) AND tablacomp='pagoordenpagocontable' AND
            impconc = rmovbancomp.bmdebito  AND fechacompr = rmovbancomp.bmfecha ; 

            IF FOUND THEN
                --RAISE NOTICE 'DENTRO DEL FOUND MINUTAAAAAAAAAAAAAA : rmovsiges (%)',rmovsiges;
    
                DELETE FROM temp_movsiges;

                DELETE FROM temp_movsiges_aux WHERE (POSITION(auxcomprob IN detalle) > 0) AND tablacomp='pagoordenpagocontable' AND impconc = rmovbancomp.bmdebito  AND fechacompr = rmovbancomp.bmfecha ;  
            ...
            ELSE
                ...
                SELECT INTO rmovsiges *
                FROM temp_movsiges_aux
                WHERE tablacomp='pagoordenpagocontable' AND
                impconc = rmovbancomp.bmdebito AND fechacompr = rmovbancomp.bmfecha; -- la fecha del comprobante debe coincidir con la fecha del banco

                IF FOUND THEN
                        --RAISE NOTICE 'DENTRO DEL FOUND : rmovimientobanco (%)',rmovimientobanco;
