CREATE OR REPLACE FUNCTION ca.liquidarvacaciones(date, integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
/*
...
BEGIN
    ...
    DROP TABLE IF EXISTS liquidacionvacacionesempleados;

    CREATE TEMP TABLE liquidacionvacacionesempleados (
             idpersona INTEGER,
             codigo INTEGER,
             laboral varchar,
             fechadesde date,
    ...
    INSERT INTO liquidacionvacacionesempleados (idpersona,codigo,laboral,fechadesde,fechafin,cantanios,canmeses,cantdias) (
     ...
     FROM(
      ...
      WHERE
           ...
           and (codigopersona=0 or codigopersona=T.idpersona)
