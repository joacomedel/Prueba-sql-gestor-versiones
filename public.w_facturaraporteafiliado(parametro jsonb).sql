CREATE OR REPLACE FUNCTION public.w_facturaraporteafiliado(parametro jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$/*
$data = array(
...
WHERE fvanc.nrofactura =parametro->>'nrofactura' AND  fvanc.tipofactura = parametro->>'tipofactura' AND fvanc.nrosucursal = parametro->>'nrosucursal' AND fvanc.tipocomprobante = parametro->>'tipocomprobante' AND nullvalue(fvanc.nrofacturafiscal) AND fvafa.tipofactura = 'FA'
...
--La factura de venta ya esta generada y solo tengo que llamar al WS.
     ...
     IF parametro->>'tipofactura' = 'NC' THEN 
		 ...
		 SELECT nrosucursal as "PtoVta",
                 CASE WHEN tipofactura = 'FA' AND tipocomprobante = 1 THEN 6
                  ...
                  FROM informefacturacionaporte 
                   NATURAL JOIN informefacturacion where idaporte = parametro->>'idaporte' AND idcentroinformefacturacion = 1 AND tipofactura = 'FA'
...
--,'20191230' as "CbteFch"
	...
	AND  tipofactura = parametro->>'tipofactura' AND nrofactura = (parametro->>'nrofactura')::bigint AND nrosucursal = (parametro->>'nrosucursal')::integer AND tipocomprobante = (parametro->>'tipocomprobante')::integer
        ;
