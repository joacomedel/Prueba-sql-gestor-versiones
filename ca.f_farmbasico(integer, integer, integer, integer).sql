CREATE OR REPLACE FUNCTION ca.f_farmbasico(integer, integer, integer, integer)
...
*/
    ...
    SET
        cemonto =T.cemonto
    FROM  (SELECT  ceporcentaje , cemonto
         FROM ca.conceptoempleado
         WHERE  idpersona =$3 and idconcepto=1028 and idliquidacion = $1
    )as T
    WHERE   idpersona =$3 and idconcepto = 1122 and idliquidacion = $1;

    -- Corresponde al monto del sueldo basico de farmacia
   SELECT INTO elmonto CASE WHEN nullvalue(cemonto) THEN 0 ELSE (cemonto) END
   FROM ca.conceptoempleado
   WHERE idliquidacion=$1 AND  idpersona =$3 AND 	idconcepto = 1028;
