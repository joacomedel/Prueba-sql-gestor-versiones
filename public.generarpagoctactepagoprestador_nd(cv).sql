CREATE OR REPLACE FUNCTION public.generarpagoctactepagoprestador_nd(character varying)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
	facturas refcursor;
...
--KR 16-03-22 TKT 4908
      SELECT INTO rdatospago  informefacturacion.*, tipocomprobanteventa.*
      FROM informefacturacion
      JOIN tipocomprobanteventa ON (tipocomprobanteventa.idtipo =informefacturacion.tipocomprobante)
   ...
   --   JOIN prestadorctacte ON (idprestador= nrocliente )
      WHERE nroinforme=elnroinforme and idcentroinformefacturacion =elidcentroinformefacturacion ;
       ...
       OPEN cfactura FOR SELECT SUM(T.importe) as importe,nroregistro, anio,iddeuda ,idcentrodeuda,idprestadorctacte,elcomprob
                         ...
                         JOIN (
                              SELECT debitofacturaprestador.importe
                                    ...
                                    ELSE   -- es un resumen
                                           comp.anioresumen
                                    END  as anio
                                    , concat(' en comprobante: ',reclibrofact.tipofactura,':',reclibrofact.letra,' ',reclibrofact.puntodeventa,'-',reclibrofact.numero) as elcomprob
                                    FROM debitofacturaprestador
                                    NATURAL JOIN informefacturacionnotadebito
                                    JOIN factura as comp  using (nroregistro,anio)
                                    JOIN reclibrofact ON (comp.nroregistro =reclibrofact.numeroregistro  and comp.anio = reclibrofact.anio)

                                    WHERE  nroinforme = elnroinforme and  idcentroinformefacturacion =elidcentroinformefacturacion

                             )as T ON(ctactedeudaprestador.idcomprobante = ((T.nroregistro*10000 ) + T.anio ))
                          group by nroregistro, anio,iddeuda ,idcentrodeuda,elcomprob;
      ...
      FETCH cfactura into lafacturadeuda;
                     ...
                     VALUES ( 54,lafacturadeuda.idprestadorctacte,  concat('Debito: ', rdatospago.tipofactura,' ',rdatospago.desccomprobanteventa,' ', rdatospago.nrosucursal,'-',rdatospago.nrofactura, 'del Comp.',lafacturadeuda.elcomprob)
