...
((not nullvalue(parametro->>'NroDocumento') AND not nullvalue(parametro->>'TipoDocumento') AND nrodoc = parametro->>'NroDocumento' AND descrip = parametro->>'TipoDocumento')
				     OR ( not nullvalue(parametro->>'NroAfiliado') AND not nullvalue(parametro->>'Barra') AND nrodoc = parametro->>'NroAfiliado' AND barra = parametro->>'Barra')
				     OR (not nullvalue(parametro->>'Track') AND parametro->>'Track' ilike concat('%',nrodoc,'_') )
				     );
		IF FOUND THEN 
        ...
        rtokenexistente = null;
                    ...
                    IF(parametro->>'uwnombre' = 'ususm'  AND parametro->>'info_solicita' = 'app' and parametro->>'accion'='Firma APP' ) THEN
                            ...
                            WHERE pt.nrodoc = nrodoctoken
                                ...
                                AND ((nullvalue(ptt.idpersonatokentipo) AND pti.ptidescripcion = 'Firma APP') OR ptt.idtipotoken = 1);
                    ...
                    IF rtokenexistente IS NULL THEN 
                        ...
                        */
                        ...
                        IF nullvalue(parametro->>'info_solicita') 
