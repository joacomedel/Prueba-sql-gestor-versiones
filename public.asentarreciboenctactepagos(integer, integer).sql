CREATE OR REPLACE FUNCTION public.asentarreciboenctactepagos(integer, integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$/*
...
OPEN cursorauxisos FOR SELECT idpv2.idinforme,idpv2.nroliquidacion,idpv2.idcargo,idpv2.importe,idpv2.fechaingreso, idpv2.nrodoc, idpv2.tipodoc, idpv2.concepto
,CASE WHEN nullvalue(TT.nrodoc) THEN 8
ELSE 13  END AS idformapago
,CASE WHEN nullvalue(TT.nrodoc) THEN 45
ELSE 48  END AS idvalcaja
,CASE WHEN nullvalue(TT.nrodoc) THEN '10311'
ELSE '20661'  END AS nrocta
,CASE WHEN nullvalue(TT.nrodoc) THEN '24917/1'
ELSE '' END AS nroctab
,CASE WHEN nullvalue(TT.nrodoc) THEN 191
...
WHERE idenviodescontarctacte ILIKE concat('%32', '' , $2 , '%' , '',$1,'%')

GROUP BY  ccd.nrodoc, ccd.tipodoc) AS TT USING(nrodoc,tipodoc)
                    WHERE idpv2.mes =$1 AND idpv2.anio=$2
                    ORDER BY nrodoc;
FETCH cursorauxisos INTO elemcursorsos;
WHILE found LOOP


SELECT INTO existerecibo * FROM recibo
                           WHERE imputacionrecibo ilike concat('%liq ',elemcursorsos.nroliquidacion,'%')
 AND imputacionrecibo ilike concat('%cargo ',elemcursorsos.idcargo,'%')
 AND imputacionrecibo ilike concat('%concepto ',elemcursorsos.concepto,'%') ;
...
--10311
     INSERT INTO recibo(idrecibo,importerecibo,fecharecibo,imputacionrecibo,centro,renrocliente, rebarra)
     VALUES (nrorecibo,elemcursorsos.importe,elemcursorsos.fechaingreso,concat('Descuento UNC liq ' , elemcursorsos.nroliquidacion , ' cargo ' , elemcursorsos.idcargo , ' ' , $1 , '/' , $2, ' concepto ', elemcursorsos.concepto),centro(), elemcursorsos.nrodoc, elemcursorsos.tipodoc);
  INSERT INTO reciboautomatico(idrecibo,centro)      VALUES (nrorecibo,centro());
    ...
    VALUES(nextval('pagos_idpagos_seq'),centro(),nrorecibo,elemcursorsos.idformapago,concat('Descuento UNC liq ' , elemcursorsos.nroliquidacion , ' cargo ' , elemcursorsos.idcargo , ' ' , $1 , '/' , $2),elemcursorsos.fechaingreso,elemcursorsos.idformapago,elemcursorsos.nrobanco,6,2,elemcursorsos.nroctab,elemcursorsos.nrocta,elemcursorsos.nroctab);
   ridpago =currval('pagos_idpagos_seq');
    ...
    /* Recuperar el pago y modificar informedescuentoplanillav2*/
      SELECT INTO regctactepago * FROM cuentacorrientepagos
...
END IF;
    INSERT INTO recibousuario (idrecibo,centro,idusuario) VALUES (elidrecibo,centro(),rusuario.idusuario) ;
---------------------------------------------------------------------------------------------------------------
