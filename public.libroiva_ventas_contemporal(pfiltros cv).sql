CREATE OR REPLACE FUNCTION public.libroiva_ventas_contemporal(pfiltros character varying)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
       rfiltros RECORD;
...
CREATE  temp TABLE temp_libroiva_ventas_contemporal
AS (
       SELECT FV.fechadesde, FV.fechahasta,
	       ...
	       ,concat(cuitini,'-',cuitmedio,'-',cuitfin) as cuit
               ...
               ,concat(tipofactura,'|',tipocomprobante,'|',nrosucursal,'|',nrofactura) as idcomprobantesiges
