CREATE OR REPLACE FUNCTION public.w_emitirconsumoafiliado_v2(parametro jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
/*
	...
	WHILE  found LOOP
		...
		ELSE
			SELECT INTO rseconsume * FROM practicavalores
						WHERE  idcapitulo = rpractica.idcapitulo AND idsubespecialidad = rpractica.idnomenclador AND idsubcapitulo = rpractica.idsubcapitulo
						AND idpractica = rpractica.idpractica  and  not internacion and  idasocconv  = rasociacion.idasocconv;

			RAISE NOTICE '(rseconsume,%)',rseconsume;
			INSERT INTO tempitems (cantidad,importe,idnomenclador,idcapitulo,idsubcapitulo,idpractica,idplancob,auditada,porcentaje,idpiezadental,idzonadental,idletradental,amuc,afiliado,sosunc) 
			VALUES(elem."Cantidad",rseconsume.importe,rpractica.idnomenclador,rpractica.idcapitulo,rpractica.idsubcapitulo,rpractica.idpractica,vidplancoberturas,null,0,'','',''
				,0,rseconsume.importe*elem."Cantidad",0);
			--Hay que generar un pedido de auditoria para esta practica
