CREATE OR REPLACE FUNCTION public.far_generastockajustedesdeprecargastockajuste()
...
BEGIN
	...
	FROM (
		select idarticulo,idcentroarticulo,sum(psaicantidadcontada) as psaicantidadcontada
		from far_precargastockajusteitem 
		where nullvalue(idstockajuste) 
                      AND psaicantidadcontada >= 0
		GROUP BY  idarticulo,idcentroarticulo 
   ...
   SELECT INTO respuesta * FROM far_abmstockajuste();
