CREATE OR REPLACE FUNCTION public.asentarreciboenctactepagos_test(integer, integer, integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$/*
...
OPEN cursorauxisos FOR 
				...
				SELECT idpv2.idinforme,idpv2.nroliquidacion,idpv2.idcargo,idpv2.importe,idpv2.fechaingreso, idpv2.nrodoc, idpv2.tipodoc, idpv2.concepto
						,CASE WHEN $3=2  THEN 8 ELSE 13  END AS idformapago
						,CASE WHEN $3=2 THEN 45 ELSE 48  END AS idvalcaja
						,CASE WHEN $3=2 THEN '10311' ELSE '20661'  END AS nrocta
						,CASE WHEN $3=2 THEN '24917/1' ELSE '' END AS nroctab
						,CASE WHEN $3=2 THEN 191 ELSE 0 END AS nrobanco
						,CASE WHEN $3=1 THEN 'SOSUNC'   WHEN $3=2 THEN 'UNC' END AS imputacionorigen
						,CASE WHEN $3=1 THEN 'SOS'   WHEN $3=2 THEN '@' END AS ladepend
                FROM informedescuentoplanillav2 as idpv2 /* 
				LEFT JOIN ( 
						SELECT ccd.nrodoc, ccd.tipodoc
						FROM cuentacorrientedeuda AS ccd 
						JOIN enviodescontarctactev2 ON(ccd.iddeuda=enviodescontarctactev2.idmovimiento AND ccd.idcentrodeuda=enviodescontarctactev2.idcentromovimiento)
						WHERE  
							CASE 
									WHEN $3 = 1 THEN  concat('32',$2 ,trim(lpad($1,2,'0')))
									WHEN $3 = 2 THEN  concat($2,trim(lpad($1,2,'0')) )
						   	END ILIKE idenviodescontarctacte
						GROUP BY ccd.nrodoc, ccd.tipodoc
				...
				LEFT JOIN ( 
					...
					FROM ctactedeudacliente  AS ccd 
				    NATURAL JOIN clientectacte  
					JOIN enviodescontarctactev2 ON(ccd.iddeuda=enviodescontarctactev2.idmovimiento AND ccd.idcentrodeuda=enviodescontarctactev2.idcentromovimiento)
					WHERE    /*reemplazo 32 por 35*/
  									-- CASE WHEN $3 = 1 THEN  concat('32',$2 ,trim(lpad($1,2,'0')))
 						CASE 
							WHEN $3 = 1 THEN  concat('35',$2 ,trim(lpad($1,2,'0')))
							WHEN $3 = 2 THEN concat($2,trim(lpad($1,2,'0')) )
						END ILIKE idenviodescontarctacte
					GROUP BY  clientectacte.nrocliente, tipodoc
				 ) AS tempfarma   USING(nrodoc,tipodoc)
				LEFT JOIN cargo ON (idpv2.idcargo = cargo.idcargo AND CASE WHEN $3=1 THEN 'SOS'= iddepen ELSE iddepen <> 'SOS' END  ) */
              	WHERE   idcargo <> 0 AND  ---- cargo <>0  
                      -- idpv2.legajosiu in (60276) AND  --04-10-2018 MALAPI: NO DEJAR PARAMETROS CABLEADOS.
                        idpv2.mes =$1 AND idpv2.anio=$2
						AND nullvalue(idpago) -- VAS 161123 Para que se quede solo con los informes que aun NO fue asentado el pago
						--KR 22-02-22 Comento y lo agrego en el join con cargo
                 		 AND CASE WHEN $3=1 THEN ('SOS'= tipoempleado OR tipoempleado='Farmacia')
                        		  ELSE not ('SOS'= tipoempleado OR tipoempleado='Farmacia') END 
      			ORDER BY nrodoc;
...
WHILE found LOOP
                        RAISE NOTICE 'Se va a ingresar la info (%)', concat('Descuento ', elemcursorsos.imputacionorigen,' liq ' , elemcursorsos.nroliquidacion , ' cargo ' ,   elemcursorsos.idcargo , ' ' , $1 , '/' , $2, ' concepto ', elemcursorsos.concepto) ;
...
,centro());
    ...
    */
    /* Recuperar el pago y modificar informedescuentoplanillav2*/
        				...
        				IF FOUND THEN  -- Se corresponde con un descuento de afiliado
        				...
        				ELSE  -- Se corresponde con descuento de cliente
                                /* Recuperar el pago y modificar informedescuentoplanillav2*/
  ...
  --END IF;
					...
					IF NOT FOUND THEN rusuario.idusuario = 25; END IF;
