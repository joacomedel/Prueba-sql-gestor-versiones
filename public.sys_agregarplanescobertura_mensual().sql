CREATE OR REPLACE FUNCTION public.sys_agregarplanescobertura_mensual()
 RETURNS void
 LANGUAGE plpgsql
...
PERFORM alta_modifica_auditoria_medica_pendientes_automaticos(nrodoc,tipodoc) FROM persona where barra = 30;
PERFORM alta_modifica_auditoria_medica_pendientes_automaticos(nrodoc,tipodoc) FROM persona where barra = 31;
PERFORM alta_modifica_auditoria_medica_pendientes_automaticos(nrodoc,tipodoc) FROM persona where barra = 32;
PERFORM alta_modifica_auditoria_medica_pendientes_automaticos(nrodoc,tipodoc) FROM persona where barra = 33 OR barra = 34 OR barra = 35 OR barra = 36;
PERFORM alta_modifica_auditoria_medica_pendientes_automaticos(nrodoc,tipodoc) FROM persona where barra = 37;
PERFORM alta_modifica_auditoria_medica_pendientes_automaticos(nrodoc,tipodoc) FROM persona where barra < 30 ;
PERFORM alta_modifica_auditoria_medica_pendientes_automaticos(nrodoc,tipodoc) FROM persona where barra > 100 AND fechafinos >= current_date ;
