CREATE OR REPLACE FUNCTION public.cambiarestadolicsinhab(boolean, date, date, integer, character varying, character varying, integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
...
usu = 'MALAPI';
  ...
  IF FOUND THEN
	...
	UPDATE persona SET fechafinos = fechafin WHERE nrodoc IN (SELECT nrodoc FROM benefsosunc WHERE nrodoctitu= $6 AND tipodoctitu = $7);
