CREATE OR REPLACE FUNCTION public.far_traercoberturasarticuloafiliado_os(character varying, bigint, bigint, bigint, integer, bigint, integer, integer)
...
UNION
	...
	WHERE 	
		(mnroregistro = $1  OR (idarticulo = trim(split_part($1,'-',1))  AND idcentroarticulo = trim(split_part($1,'-',2)))) 
		AND idvalidacion = $5 
