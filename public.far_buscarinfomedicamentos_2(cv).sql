CREATE OR REPLACE FUNCTION public.far_buscarinfomedicamentos_2(character varying)
...
,case when nullvalue(a.adescripcion) then concat(mnombre , ' ' , mpresentacion , ' $ ' , vm.vmimporte , ' ' ,me.mtroquel)
                   ELSE concat(a.adescripcion , ' $ ' , p.pavalor) END as detalle
,l.lstock  
...
LEFT JOIN far_articulo as a on a.acodigobarra = me.mcodbarra::text
LEFT join tipoiva as iva using(idiva)
LEFT JOIN far_precioarticulo as p on a.idarticulo=p.idarticulo 	and nullvalue(p.pafechafin) 
left join far_medicamento as m on a.idarticulo=m.idarticulo  
left join far_rubro as ra ON (ra.idrubro = a.idrubro OR (nullvalue(a.idrubro) AND ra.idrubro = 4))  
left join (select idarticulo,sum(lstock) as lstock  from far_lote  GROUP BY idarticulo ) as l on a.idarticulo = l.idarticulo  
WHERE (concat(me.mnombre,me.mcodbarra::text,me.mtroquel,me.mnroregistro::text) ilike  $1)
UNION 
...
,case when nullvalue(a.adescripcion) then concat(mnombre , ' ' , mpresentacion , ' $ ' , p.pavalor , ' ' ,me.mtroquel)
ELSE concat(a.adescripcion , ' $ ' , p.pavalor) END as detalle
,l.lstock  
,me.mtroquel as troquel
,me.mpresentacion as presentacion
...
,p.pafechaini
FROM far_articulo as a  LEFT JOIN far_medicamento as m on a.idarticulo=m.idarticulo   LEFT JOIN medicamento as me on a.acodigobarra = me.mcodbarra::text
...
left join (select idarticulo,sum(lstock) as lstock  from far_lote  GROUP BY idarticulo ) as l on a.idarticulo = l.idarticulo  
WHERE (concat(a.acodigobarra,adescripcion,rdescripcion,a.idarticulo::text,'-',a.idcentroarticulo)  ilike  $1)
$function$
