CREATE OR REPLACE FUNCTION public.datoscuponessordenados(bigint, character varying, date, date)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
DECLARE
...
--Busco el titular en person, segun la barra que han mandado
 FOR rec IN SELECT DISTINCT tipodoc, fechafinos, idafiliado, barra, nrodoc  FROM persona
	 ...
	 NATURAL JOIN (SELECT tipodoc,nrodoc,idlocalidad,idprovincia,cargo.fechafinlab FROM cargo
        ...
        /*  AND (nrodoc, tipodoc) not in (select nrodoc, tipodoc from cargo
    where (cargo.idcateg < '9' and cargo.idcateg > '1')
    OR  cargo.idcateg = 'JTPE'  OR  cargo.idcateg = 'TITE'
    OR  cargo.idcateg = 'ASOE'
    OR idcateg = 'CITE' OR idcateg = 'ADJH'  OR idcateg = 'CSOE'
    OR idcateg = 'CDJE'
                                            )*/
