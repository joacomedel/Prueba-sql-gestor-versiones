CREATE OR REPLACE FUNCTION public.gestionarcuponesmasivos(character varying, integer, integer, date, date)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
DECLARE
...
elnrodoc =$1;
     ...
     --creo la temporal
      CREATE TEMP TABLE temporalcupones (
           ...
           WHERE persona.barra=barraafi and persona.nrodoc=elnrodoc and idcentrotarjeta=elcentro
