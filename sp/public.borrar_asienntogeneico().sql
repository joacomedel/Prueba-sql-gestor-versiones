CREATE OR REPLACE FUNCTION public.borrar_asienntogeneico()
...
BEGIN
     OPEN curasiento FOR SELECT * FROM asientogenerico WHERE idasientogenerico = 40310 ;
...
--RAISE NOTICE 'tipocomprobante (%)',split_part(regasiento.idoperacion, '|', 2);
		...
		if found then
			...
			WHILE FOUND LOOP
				...
				values(xidasiento,centro(),xitem,regitem.idconcepto,regencabezado.concepto,xd_h);						
				xitems = xitems + xitem;
			...
			WHILE FOUND LOOP
				...
				insert into asientogenericoitem(idasientogenerico,idcentroasientogenerico,acimonto,nrocuentac,acidescripcion,acid_h)
				values(xidasiento,centro(),regitem.monto,regitem.nrocuentac,regencabezado.concepto,xd_h);						
