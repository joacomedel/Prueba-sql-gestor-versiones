CREATE OR REPLACE FUNCTION public.alta_modifica_preauditoria_odonto()
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
  respuesta BOOLEAN;
     ...
     IF elem.tipo = 14 or elem.tipo= 37 THEN 
      ...
      SELECT INTO elemriexiste * FROM recetarioitem WHERE centro = elem.centro and nrorecetario = elem.nroorden and mnroregistro = elem.iditem;
        ...
        IF FOUND THEN 
                    ...
                    WHERE nrorecetario=elemriexiste.nrorecetario AND centro= elemriexiste.centro AND idrecetarioitem=elemriexiste.idrecetarioitem and idcentrorecetarioitem=elemriexiste.idcentrorecetarioitem;
        ELSE 
