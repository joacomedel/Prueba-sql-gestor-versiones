CREATE OR REPLACE FUNCTION ca.f_farmactaacuerdo072022(integer, integer, integer, integer)
...
BEGIN
 ...
 date_trunc('month', concat(rliquidacion.lianio,'-',rliquidacion.limes,'-1')::date) <= cefechafin or nullvalue(cefechafin)
   ...
   /*esto lo busco para saber si la persona trabaja jornada completa o no y si trabajo los 30  dias o no*/                  
  ...
  /*Segun mail del 22062023 de JE se ratfica que debe ser considerado el proporcional*/
  IF  (rconcepto.cemonto <> rconcepto.ceporcentaje and (rconcepto.cemonto/rconcepto.ceporcentaje)<0.75) --jornada reducida
   ...
   select into elmontotope * 
         ...
         WHERE  idconcepto = 1278 and idcategoria=rcatemp.idcategoria and nullvalue(ctfechahasta);
		...
		if(rcatemp.idcategoria=17 or rcatemp.idcategoria=18) then -- Categoría 17  Farmaceutico y  Categoria 18 Farmaceutico Auxiliar
			 elmonto =elmontotope.ctmontominimo    * proporcion;  
                 end if;
