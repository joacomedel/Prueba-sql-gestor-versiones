CREATE OR REPLACE FUNCTION public.far_asentarfacturaventav2()
...
CREATE TEMP TABLE tempfacturaventa ( tipocomprobante INTEGER , nrosucursal INTEGER , nrofactura BIGINT , nrodoc VARCHAR, tipodoc SMALLINT, ctacontable INTEGER, centro INTEGER , tipofactura VARCHAR(2), barra BIGINT , idusuario BIGINT  );
CREATE TEMP TABLE temitemfacturaventa ( idconcepto  VARCHAR , cantidad INTEGER , importe DOUBLE PRECISION , descripcion VARCHAR, idiva INTEGER,iditemcc INTEGER );
...
CREATE TEMP TABLE tempfacturaventacupon ( idvalorescaja INTEGER ,  autorizacion VARCHAR ,  nrotarjeta VARCHAR,  monto DOUBLE PRECISION ,  montodto DOUBLE PRECISION ,  cuotas SMALLINT ,  fvcporcentajedto DOUBLE PRECISION ,  nrocupon VARCHAR);
 ...
 INSERT INTO tempfacturaventa (tipocomprobante, nrosucursal,  nrofactura, nrodoc ,    tipofactura , barra,idusuario) VALUES (1,2,138053,'38082162','FA',1,119);
INSERT INTO temporden (nroorden, centro, idcomprobantetipos) VALUES (49800,1,44);
INSERT INTO tempfacturaventacupon (idvalorescaja  ,autorizacion,nrotarjeta ,monto ,cuotas,nrocupon,fvcporcentajedto,montodto) VALUES (5,'00113703','06800','138.15','1','002923','0.0','0.00');
    ...
    /* if (not nullvalue( rfactventa.importedescuento) AND rfactventa.importedescuento >0)THEN
     ...
     IF (tipoorden=44) THEN --   Se esta facturando una orden de farmacia
                 ...
                 WHILE FOUND LOOP
                        IF (elidarticulo <> unitemfact.idarticulo) THEN
