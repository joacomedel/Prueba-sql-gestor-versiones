CREATE OR REPLACE FUNCTION public.sys_modificar_pagocoseguro_orden(parametro jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
/*
* El objetivo de este SP es generar la modificación en el pago del coseguro de un consumo registrado como efectivo.
Si el consumo on-line estaba configurado en la tabla   w_usuariowebprestador como uwpformapagotipodefecto =2 y el prestador NO REALIZO el cobro del coseguro
esto se modifica para que el afiliado pague el coseguro en cuenta corriente
