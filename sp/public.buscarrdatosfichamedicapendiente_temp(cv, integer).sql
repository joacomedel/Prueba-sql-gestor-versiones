CREATE OR REPLACE FUNCTION public.buscarrdatosfichamedicapendiente_temp(character varying, integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
DECLARE
...
OPEN cursorficha FOR SELECT fichamedicaitem.idfichamedica,fichamedicaitem.idcentrofichamedica,nrodoc, tipodoc,
              ...
              (SELECT sum(fichamedicaitem.fmicantidad) as cantidad,fichamedicaitem.idnomenclador,fichamedicaitem.idcapitulo, 	
                            ...
                            --Tomo las auditorias solo de este anio,puesto que la auditoria psicoterapia inicia de cero cada nuevo anio
              ...
              LEFT JOIN (select sum(t.cantemitida) as cantemitida,t.idnomenclador,
                        ...
                        AND nullvalue(ordenestados.centro)
                         -- Tomo el consumo solo del anios en el que estamos, puesto que la auditoria psicoterapia inicia de cero cada nuevo anio
                          -- Tomo el consumo solo del anios en el que estamos, puesto que la auditoria psicoterapia inicia de cero cada nuevo anio
...
WHILE  found  LOOP
                ...
                IF (regficha.cantresta <= 0) THEN
                   BEGIN
                     ...
                     INSERT INTO ttfichamedicaemisionpendiente_temp VALUES(
                     regficha.fmepfecha,
