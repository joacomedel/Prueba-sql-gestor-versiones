CREATE OR REPLACE FUNCTION public.sys_auditar_suap(pparametros character varying)
...
--MaLaPi 16-06-2022 Modifico para que llame al mismo SP que llama la ventana de Java para normalizar los datos
SELECT INTO vrespuesta sys_auditar_suap_previo(pparametros);
...
vbandera = true;
--Para Cada Orden
OPEN ccursororden FOR SELECT CASE WHEN not nullvalue(plana.idprestador) THEN plana.idprestador ELSE planb.idprestador END as idprestador
...
,CASE 
 --WHEN NOT nullvalue(categoriaefector) AND trim(categoriaefector) <> '' THEN categoriaefector
                                  WHEN NOT nullvalue(categoria_efector) AND trim(categoria_efector) <> '' THEN categoria_efector  
			...
			LEFT JOIN prestador as plana  ON idprestador = nromatricula
                        ...
                        --AND  nroorden  NOT IN (1149251,1150108,1150294,1150404) 
...
WHILE  found LOOP 
		...
		IF nullvalue(elem.idfichamedicapreauditada) OR elemorden.cant > 1 THEN 
			...
			vimporte = CASE WHEN nullvalue(elem.importexcategoria) THEN elem.importepv  ELSE elem.importexcategoria END;
                        ...
                        --24-11-2021 MaLaPi Intuyo que es el valor anterior el que debo tomar, solo si la diferencia de los valores es mayor a 1.01, ademas de las condiciones que ya tenia    
                        ...
                        IF rfiltros.confiarenvalorpractica = 'si' AND nullvalue(elem.convanteriorfechafin) AND not nullvalue(elemorden.valor_practica) AND abs(vimporte- elemorden.valor_practica) > 1.1 THEN 
                        ...
                        -- MaLaPi 22-11-2021 Si confiamos en el valor de la practica, y la diferencia es menor a 0.1 entre lo cargado y lo que envia el prestado, usamos lo que envia el prestador. 
                         IF rfiltros.confiarenvalorpractica = 'si' AND not nullvalue(elemorden.valor_practica) AND (vimporte + 0.1) > elemorden.valor_practica THEN
                           RAISE NOTICE 'SYS::RN Colegio Mendico factura menos o con una diferencia menor a 0.1 (%),(%)',vimporte,elemorden.valor_practica; 
