CREATE OR REPLACE FUNCTION public.insertarreintegro(integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
	...
	WHILE  found LOOP
       ...
       SELECT INTO rcuentas * FROM cuentas  WHERE rrec.nrodoc = cuentas.nrodoc AND rrec.tipodoc = cuentas.tipodoc;
 	    INSERT INTO reintegro (anio,idcentroregional,tipodoc,nrodoc,tipocuenta,nrocuenta,tipoformapago,nroordenpago,rimporte,rfechaingreso,idrecepcion,nrooperacion)
             VALUES (rrec.anio,rrec.idcentroregional,rrec.tipodoc,rrec.nrodoc,rcuentas.tipocuenta,rcuentas.nrocuenta,null,null,null,rrec.fecha,idrec,null);
