CREATE OR REPLACE FUNCTION public.imputarpagoctactesindeudaprevia()
 RETURNS SETOF recibo
 LANGUAGE plpgsql
AS $function$
/* Funcion que dado un pago y un grupo de deudas genera las imputaciones correspondientes. */
DECLARE
rusuario record;
...
ELSE 
	...
	IF rorigenctacte.origentabla = 'nolose' THEN 
		...
		SELECT INTO runadeuda * FROM temppagodeuda NATURAL JOIN cuentacorrientedeuda LIMIT 1;
		vorigen = 'afiliado';
		UPDATE temppagodeuda SET origendeuda = vorigen;
		SELECT INTO vimputacionrecibo text_concatenar(concat(movconcepto , '-'))
                                   FROM temppagodeuda
...
--KR 18-03-21 Verifico si la deuda tiene saldo 
     ...
     WHILE  found LOOP
           ...
           --Guardo en la nueva estructura de cuentas corrientes DEUDA PAGO Y UPDATEO LA DEUDA INSTITUCION 
               ...
               VALUES (vmovpago,centro(),unmovimiento.iddeuda,unmovimiento.idcentrodeuda,CURRENT_TIMESTAMP, round(unmovimiento.importeapagar::numeric,2));

               UPDATE cuentacorrientedeuda SET saldo =   round(CAST (cuentacorrientedeuda.saldo-unmovimiento.importeapagar AS numeric), 2)
...
--KR 18-03-21 Verifico si la deuda tiene saldo 
	     OPEN cursormovimientos FOR SELECT * FROM temppagodeuda NATURAL JOIN ctactedeudacliente WHERE saldo >0;
	     FETCH cursormovimientos into unmovimiento;
