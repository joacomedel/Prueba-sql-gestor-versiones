CREATE OR REPLACE FUNCTION public.agregaraportes()
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$/*
...
--MaLaPi 05-11-2019 No se toca la tabla persona, no se deberia disparar ningun trigger
--ALTER TABLE  persona DISABLE TRIGGER ALL;
--ALTER TABLE persona disable trigger actualizaestadoconfechafinos;


--agrego los aportes de la sosunc

  SELECT INTO resultado2 * FROM agregaraportes_sosunc();

--genero el informe de aportes y contribuciones
/*
select INTO infaporte * from generarinformefacturacionaportescontribuciones(date_part('month', current_date -30)::integer, date_part('year', current_date -30)::integer);
*/
...
OPEN cursorauxi FOR SELECT legajosiu,nroliquidacion,nrocargo
              ...
              /*KR 21-4-15 agregue a los afiliados de sosunc para usar el sp y procesar los aportes de los afil de sosunc*/
                     ...
                     (SELECT case when (elcargo.idcargo<990000) then 
                       (990000 +elcargo.idcargo::numeric)  else  elcargo.idcargo end  as legajosiu,
...
/*KR 21-4-15 agregue a los afiliados de sosunc para usar el sp y procesar los aportes de los afil de sosunc*/
         UNION
          SELECT * FROM
         (SELECT case when (elcargo.idcargo<990000) then (990000 +elcargo.idcargo::numeric)  else  elcargo.idcargo end  as legajosiu,
             ca.persona.penrodoc as nrodoc , ca.persona.idtipodocumento as tipodoc 
             FROM ca.empleado  NATURAL JOIN ca.persona JOIN ( SELECT nrodoc,tipodoc,MAX(idcargo)as idcargo,legajosiu		
                                                    FROM cargo WHERE fechainilab<= CURRENT_DATE 
                                                    and fechafinlab >=	CURRENT_DATE and iddepen='SOS'		
...
/* KR comento 31-03-15 porque ahora se creo un sp desde donde se procesan los descuentos en cta cte de todos los empleados
SOSUNC Y UNCo
SELECT into resultado * FROM agregardescuentosconceptos();
*/
return resultado;
