CREATE OR REPLACE FUNCTION public.controles_consumopracticacantidad_contemporal(pfiltros character varying)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
...
*/
 IF iftableexists('temp_controles_consumopracticacantidad_contemporal') THEN
     DROP  TABLE temp_controles_consumopracticacantidad_contemporal;
 END IF;


CREATE TEMP TABLE temp_controles_consumopracticacantidad_contemporal
AS (
	SELECT DISTINCT extract(YEAR FROM age(t.fechaemision, persona.fechanac)) as edad ,persona.sexo,persona.nombres,persona.apellido,persona.nrodoc,t.idsubespecialidad,t.idcapitulo,t.idsubcapitulo,t.idpractica,pdescripcion,descripcion,nrocuentac,desccuenta,cregional,pp.acdecripcion,idprestador, elprestador, t.idasocconv,pv.importe,importeemision,cantidad, t.laorden,
...
extract(YEAR FROM fechaemision) as año, extract(MONTH FROM fechaemision) as mes,extract(DAY FROM fechaemision) as dia 

	  ,'1-Edad#edad@2-Sexo#sexo@3-Nombres#nombres@4-Apellido#apellido@5-Nrodoc#nrodoc@6-Nomenclador#idsubespecialidad@7-Capitulo#idcapitulo@8-SubCapitulo#idsubcapitulo@9-Practica#idpractica@10-Nombre#pdescripcion@11-Plan Cobertura#descripcion@12-Centro Regional#cregional@13-Asoc.#acdecripcion@14-ID_Prestador#idprestador@15-Prestador#elprestador@16-Importe Convenio#importe@17-Cantidad#cantidad@18-Importe Emision#importeemision@19-Nro.Orden#laorden@20-FechaEmision#fechaemision@21-Año#año@22-Mes#mes@23-Dia#dia@24-Titular#titular'::text as mapeocampocolumna 
	     FROM
		(SELECT idprestador, elprestador,  nrodoc,0.01 as importeemision,'12' as idsubespecialidad, '42' AS idcapitulo,'01' AS idsubcapitulo, '01' AS idpractica,ctdescripcion,   'CONSULTA MEDICA(B)' as pdescripcion,   '' as descripcion,'' as nrocuentac,'Ordenes Médicas' as desccuenta,'' as idplancobertura,orden.idasocconv,concat( to_char(centroregional.idcentroregional,'99'),' - ' , centroregional.crdescripcion)  As CRegional, count(ordconsulta.nroorden) as cantidad,max(fechaemision) as fechaemision, text_concatenar(concat(orden.nroorden,'-',orden.centro)) as laorden
		FROM orden 
                LEFT JOIN (SELECT idprestador,pdescripcion as elprestador, nroorden,centro ,tipo FROM ordenesutilizadas JOIN prestador USING (idprestador) )as p USING ( nroorden,centro ,tipo) 
		...
		GROUP BY idprestador, elprestador,  nrodoc,orden.idasocconv,idsubespecialidad,idcapitulo,idsubcapitulo,idpractica,ctdescripcion,pdescripcion,descripcion,nrocuentac,desccuenta,    
			idcentroregional,crdescripcion,itemvalorizada.idplancovertura
	) as t    
    ...
    IF iftableexists_fisica('temp_consumo_practicas_290824') THEN
           INSERT INTO temp_consumo_practicas_290824 (edad,	sexo,	nombres,	apellido,	nrodoc,	idsubespecialidad,	idcapitulo,	idsubcapitulo,	idpractica,	pdescripcion,	descripcion,	nrocuentac,	desccuenta,	cregional,	acdecripcion,	idprestador	,elprestador,	idasocconv,	importe,	importeemision,	cantidad,	laorden,	titular,	fechaemision)(
                 SELECT edad,	sexo,	nombres,	apellido,	nrodoc,	idsubespecialidad,	idcapitulo,	idsubcapitulo,	idpractica,	pdescripcion,	descripcion,	nrocuentac,	desccuenta,	cregional,	acdecripcion,	idprestador	,elprestador,	idasocconv,	importe,	importeemision,	cantidad,	laorden,	titular,	fechaemision
               FROM temp_controles_consumopracticacantidad_contemporal 
           );
