CREATE OR REPLACE FUNCTION public.datospadronv3(fechacierre date)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
...
WHILE FOUND loop
	...
	if(pers.barra=30 or pers.barra=31) then 
		select into veranio * from verificarcargoininterrumpido(pers.nrodoc, pers.tipodoc, fechacierre, 365);
                if veranio then
