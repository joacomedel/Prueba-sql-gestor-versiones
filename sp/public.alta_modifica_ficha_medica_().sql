CREATE OR REPLACE FUNCTION public.alta_modifica_ficha_medica_()
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
DECLARE
    ...
    IF not nullvalue(elem.infomedicamentos) AND (elem.infomedicamentos) THEN  -- Se requiere generar Seguimiento de Medicamentos
       ...
       IF not nullvalue(elem.idsolicitudauditoriaitem) THEN  
			...
			IF NOT FOUND THEN 
				...
				VALUES(rsolicituditem.idsolicitudauditoria,rsolicituditem.idcentrosolicitudauditoria,null,sys_dar_usuarioactual(),2,'Al generar los items de la auditoria',elem.fmidescripcion);
			END IF;
		...
		/*IF not nullvalue(elem.generarpendientes) THEN
			--MaLaPi 23-09-2022 Verifico si ya no se habia generado un pendiente
			  ...
			  INSERT INTO temp_alta_modifica_alcance_cobertura (accion,nrodoc,tipodoc,fecha_desde,fecha_hasta,periodo
				...
				,idnomenclador,idcapitulo,idsubcapitulo,idpractica,idusuario,fmiporreintegro ) 
				VALUES('ConfiguraEmisiones',elem.nrodoc,elem.tipodoc,elem.fmimfechafin,elem.fmimfechafin,elem.periodo
					   ...
					   ,elem.prioridad,elem.seaudita,elem.serepite,elem.cantidad,elem.fmimcobertura,elem.cantidadtotal,elem.acidplancoberturas
...
/*
  IF iftableexistsparasp('temppaseinfodocfichamedicainfomedicamento') THEN 
     ...
     IF nullvalue(rpidfmim.idpaseinfodocfichamedicainfomedicamento) THEN 
        INSERT INTO paseinfodocfichamedicainfomedicamento(idpaseinfodocumento,idcentropaseinfodocumento,idfichamedicainfomedicamento,idcentrofichamedicainfomedicamento)             
        SELECT idpaseinfodocumento,idcentropaseinfodocumento,idfichamedicainfomedicamento,idcentrofichamedicainfomedicamento FROM temppaseinfodocfichamedicainfomedicamento;  
     ...
     ELSE 
       ...
       FROM (   SELECT idpaseinfodocumento,idcentropaseinfodocumento,idfichamedicainfomedicamento,idcentrofichamedicainfomedicamento,idpaseinfodocfichamedicainfomedicamento,idcentropaseinfodocfichamedicainfomedicamento
