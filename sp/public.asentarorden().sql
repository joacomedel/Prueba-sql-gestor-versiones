CREATE OR REPLACE FUNCTION public.asentarorden()
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
DECLARE
...
/*
nrodoc        varchar
tipodoc       integer        NUEVO CAMPO
numorden     bigint       NUEVO CAMPO
ctroorden                    NUEVO CAMPO
centro        integer
idasocconv     bigint
...
BEGIN
    ...
    if dato.cantordenes is null then
       cc=1;
    else
        cc = dato.cantordenes;
    end if;
    ...
    while contador>0 loop	
        --asienta en orden
        	...
        	INSERT INTO orden(nroorden,centro,fechaemision,tipo,idasocconv,nroordeninter,centroordeninter)
                   VALUES (nextval('"public"."orden_nroorden_seq"'),dato.centro,CURRENT_TIMESTAMP,dato.tipo,dato.idasocconv,dato.numorden,dato.ctroorden);
            resp = currval('"public"."orden_nroorden_seq"');
	      
	--aienta en consumo
            INSERT INTO consumo(idconsumo,centro,nroorden,nrodoc,tipodoc)
                    VALUES (nextval('"public"."consumo_idconsumo_seq"'),dato.centro,resp,dato.nrodoc,dato.tipodoc);	
    --asienta en importesorden
            if not dato.amuc is null AND (dato.amuc <> 0) then
            ...
            if not dato.efectivo is null  AND (dato.efectivo <> 0) then
            ...
            if not dato.cuentacorriente is null  AND (dato.cuentacorriente <> 0) then
           ...
           if not dato.debito is null  AND (dato.debito <> 0) then
            ...
            if not dato.credito is null  AND (dato.credito <> 0) then
            ...
            if not dato.sosunc is null  AND (dato.sosunc <> 0) then
                 INSERT INTO importesorden(nroorden,centro,idformapagotipos,importe)
