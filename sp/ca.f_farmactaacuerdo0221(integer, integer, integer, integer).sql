CREATE OR REPLACE FUNCTION ca.f_farmactaacuerdo0221(integer, integer, integer, integer)
...
BEGIN
 ...
 date_trunc('month', concat(rliquidacion.lianio,'-',rliquidacion.limes,'-1')::date) <= cefechafin or nullvalue(cefechafin)
   ...
   /*esto es par abuscar si la persona trabajo jornada completa o no */                  
    ...
    if found then    
              ...
              /*Por pedido de JE segun comentario de arriba, debe abonarse completo*/   
         /*    IF  ((rconcepto.cemonto <> rconcepto.ceporcentaje 
                 and (rconcepto.cemonto/rconcepto.ceporcentaje)<0.75))
                    THEN               
                         --   proporcion = 0.66625;
   ...
   select into elmontotope * FROM ca.conceptotope  
                                             WHERE  idconcepto = 1263 and idcategoria=rcatemp.idcategoria
                                          and nullvalue(ctfechahasta);
/*
Ayudante en gestión de Farmacia $4965.52
Personal en Gestión de Farmacia $6075
Farmacéutica $6721.54
*/
