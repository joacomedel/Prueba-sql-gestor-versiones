CREATE OR REPLACE FUNCTION public.calcularvalorespracticaxcategoria(pechaingreso timestamp without time zone)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
...
OPEN alta FOR select nomen.idnomenclador
				...
				THEN practconvvalcantidad.cantidadh1 ELSE nomen.cantidad1 END as cantidad1
                     ...
                     WHERE practconvval.idasocconv <> 154  --MaLaPi 06-09-2022 Saco Coseguros... 
					  AND practconvval.tvvigente AND pcvfechaingreso ilike concat('%',pechaingreso,'%');
       ...
       IF (importeprac >= 0) THEN
         ...
         IF FOUND THEN
                  IF (to_number(to_char(rpracticavalor.importe, '9999999999.99'),'9999999999.99') <> to_number(to_char(importeprac, '9999999999.99'),'9999999999.99')) THEN
                   ...
                   --MaLaPi 06-10-2022 Se empieza a guardar el orden de los historicos para saber cuales son los ultimos N. 
                     UPDATE practicavaloresxcategoriahistorico SET pvxchordenhistorico = pvxchordenhistorico + 1 WHERE idasocconv = to_number(elem.idasocconv,'99999999')
                                                                 AND idsubespecialidad = elem.idnomenclador
                                                                 AND idcapitulo = elem.idcapitulo
                                                                 AND idsubcapitulo = elem.idsubcapitulo
                                                                 AND idpractica = elem.idpractica
                   ...
                   --Guardo en el Historico: el nuevo se deja como el ultimo historico, es decir, con fecha pvxchfechafin en null y el anterior se coloca fecha actual. 
                     UPDATE practicavaloresxcategoriahistorico SET pvxchfechafin = now(),pvxchfechafinvigencia = elem.pcvfechainicio  WHERE idasocconv = to_number(elem.idasocconv,'99999999')
                       ...
                       VALUES (rcateg.pcategoria,to_number(elem.idasocconv,'99999999'),elem.idnomenclador,elem.idcapitulo,elem.idsubcapitulo,elem.idpractica,to_number(to_char(rpracticavalor.importe, '9999999999.99'),'9999999999.99'),elem.internacion,elem.pcvfechainicio,rusuario.idusuario);
                       --MaLaPi 06-09-2022 Si el valor de la practica calculado es cero (0) ya no esta configurado para esa asociacion
					   IF (importeprac > 0) THEN
                       		UPDATE practicavaloresxcategoria SET pvxcfechaini = now(), importe = to_number(to_char(importeprac, '9999999999.99'),'9999999999.99'),pvxcidusuario = rusuario.idusuario, pvxcfechainivigencia = elem.pcvfechainicio  WHERE idasocconv = to_number(elem.idasocconv,'99999999')
                                                                 AND idsubespecialidad = elem.idnomenclador
                                                                 AND idcapitulo = elem.idcapitulo
                                                                 AND idsubcapitulo = elem.idsubcapitulo
                                                                 AND idpractica = elem.idpractica
         ...
         ELSE
		        IF importeprac > 0 THEN   
                  	...
                  	VALUES (rcateg.pcategoria,to_number(elem.idasocconv,'99999999'),elem.idnomenclador,elem.idcapitulo,elem.idsubcapitulo,elem.idpractica,to_number(to_char(importeprac, '9999999999.99'),'9999999999.99'),elem.internacion,elem.pcvfechainicio,rusuario.idusuario);
        	     END IF;
		END IF;

     END IF;
     --RAISE NOTICE 'calcularvalorespracticaxcategoria: Listo la categoria (%) rcateg  ',rcateg;
     FETCH curcateg INTO rcateg;
     END LOOP;
     CLOSE curcateg;
RAISE NOTICE 'calcularvalorespracticaxcategoria: Listo (%) elem ',elem;
FETCH alta INTO elem;
errores = FALSE;
END LOOP;
CLOSE alta;                  	
