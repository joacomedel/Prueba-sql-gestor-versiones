CREATE OR REPLACE FUNCTION public.far_traerinfocoberturasnueva(character varying)
...
WHILE  found LOOP
    ...
    */
    ...
    WHERE  persona.nrodoc = vnrodoc /*AND tipodoc = vtipodoc*/ AND fechafinos >=  current_date /*- 30::integer*/
    AND ( nullvalue(eafechahasta) OR NOT current_date <= eafechahasta)
    AND ( nullvalue(eafechadesde) OR NOT eafechadesde <= current_date);
    IF FOUND THEN
    ...
    for rcobinfomedi in 
        ...
        SELECT 
              ...
              CASE WHEN (idobrasocial=1 AND restante < vi.cantidadaprobada ) THEN restante
              ELSE  
                CASE WHEN restante < vi.cantidadaprobada THEN vi.cantidadaprobada-restante ELSE vi.cantidadaprobada END 
              END  as cantidadaprobada,
              idarticulo ,
              idcentroarticulo ,
              idrubro ,
              adescripcion ,
              CASE WHEN (idobrasocial=1001) THEN importeunitario ELSE precio END as precio,  ---- 281024 Alba y Facu solo cuando es PAMI obtenemos el precio dado en la validacio
              rdescripcion ,
              astockmin ,
              astockmax ,
              acomentario ,
        ...
        FROM (
            SELECT *
            FROM far_traercoberturasarticuloafiliado_NUEVA
