CREATE OR REPLACE FUNCTION public.movimientosctacteprestamoasistencial(date, date, character varying)
 RETURNS SETOF movimientoctacteprestamoasis
 LANGUAGE sql
AS $function$
SELECT
...
,'Sin Facturas'::text as infofacturas
...
WHERE  d.idcomprobantetipos = 18 OR d.idcomprobantetipos = 17 --Prestamos Asistencial
...
AND pc.idcomprobantetipos = 18 --Prestamos Asistencial


UNION

SELECT
fechamovimiento::varchar,idconcepto,importedeuda,importepagado,fechapagoprobable::varchar,fechapago::varchar,iddeuda,idcentrodeuda
