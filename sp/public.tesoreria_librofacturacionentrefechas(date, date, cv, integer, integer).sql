CREATE OR REPLACE FUNCTION public.tesoreria_librofacturacionentrefechas(date, date, character varying, integer, integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
	...
	pidprestador alias for $5; --Si es cero, quiere decir que no se filtra prestador. 
...
, mesanio VARCHAR, auditado VARCHAR, pagado VARCHAR, tipoprestador VARCHAR,pcuit VARCHAR/*,idasientocontable VARCHAR*/, debitoobs VARCHAR, agfechacontable_registro VARCHAR
);
...
, mesanio, auditado, pagado, tipoprestador,pcuit /*,idasientocontable*/, debitoobs, agfechacontable_registro
) 
VALUES('15-Nro. Operacion','14-OP. Multivac','11-Minuta de Pago','5-Factura',
'18-Notas de Debito','2-Nro.Registro','17-Imp.Operacion','8-A Pagar','7-Debito','6-Importe','20-Forma Pago'
,'16-Cta. Sosunc','4-Observaciones','3-Prestador','12-Fecha MP','13-Fecha.Operacion','10-F.Vto.Fact.','9-F.Emision Fact.','1-Recepcion','21-Fecha Cont', '19-Fecha Emision ND', '22-Id. Prestador'
, '23-MMAA', '24-Auditado', '25-Pagado', '26-Tipo Prestador','27-CUIT'/*,'27-ID Asiento Contable'*/,'28-Motivo Debito'
,'29-Fecha_Contable_Registro'
);
...
SELECT 
      ...
      /* ,CASE WHEN nullvalue(ag.idasientogenerico) THEN ' ' ELSE concat(ag.idasientogenerico,'|',ag.idcentroasientogenerico) END AS idasientocontable*/
        ,debitoobs 
        ,asiento_rlf.agfechacontable as agfechacontable_registro
FROM recepcion 
...
)as asiento_rlf ON(nullvalue(idasientogenericorevertido) 
                   AND idcomprobantesiges=concat(rlf.numeroregistro,'|',rlf.anio)
                   AND idasientogenericocomprobtipo = 7	
                   AND agdescripcion not ilike '%REVERSION%'  )  --- VAS Y BelenA LEFT incorporado el 080425
...
LEFT JOIN (SELECT CASE WHEN nullvalue(idresumen) THEN  f.nroregistro ELSE idresumen END as nroregistro
              ...
              , text_concatenar(debitoobs) as debitoobs
            FROM factura as f 
/*Agrego Dani 2015-09-07*/
   ...
   --KR 21-03-19  tipoestadofactura = 1 AND agfechacontable>='2019-01-01' son confiables. Los previos no (hablado con CS)
	    ...
	    LEFT JOIN (SELECT  nroregistro,anio,concat(tipofactura , ' ' , to_char(nrosucursal, '0000') , '-' ,  to_char(nrofactura, '00000000')) AS nrofacturaconformato
				               ,sum(importectacte)  as debito, fechaemision as fenotadebito,debitoobs
		       FROM (
		            SELECT nroregistro,anio,nroinforme,idcentroinformefacturacion, text_concatenar(concat(observacion,' ',motivo)) debitoobs
		            FROM  debitofacturaprestador
			    NATURAL JOIN informefacturacionnotadebito 
                            LEFT JOIN facturadebitoimputacionpendiente USING(nroregistro, anio ) 
			...
			GROUP BY fenotadebito,debitoobs,nroregistro,anio,concat(tipofactura , ' ' , to_char(nrosucursal, '0000') , '-' ,  to_char(nrofactura, '00000000'))
...
/*Agrego Dani 2015-09-07*/
   ...
   --KR 21-03-19  tipoestadofactura = 1 AND agfechacontable>='2019-01-01' son confiables. Los previos no (hablado con CS)
	            ...
	            LEFT JOIN (SELECT  nroregistro,anio,concat(tipofactura , ' ' , to_char(nrosucursal, '0000') , '-' ,  to_char(nrofactura, '00000000')) AS nrofacturaconformato
				                   ,sum(importectacte)  as debito,debitoobs
			              FROM (
				               SELECT nroregistro,anio,nroinforme,idcentroinformefacturacion, text_concatenar(concat(observacion,' ',motivo)) debitoobs
		            FROM  debitofacturaprestador
			    NATURAL JOIN informefacturacionnotadebito 
