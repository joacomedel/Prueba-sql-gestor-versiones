CREATE OR REPLACE FUNCTION public.dardatoscuentaproveedor_ineficiente()
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
...
--KR 04-06-19 busco los datos de las MP que generaron movimientos para los afiliados 
	...
	IF FOUND THEN 
        ...
        UPDATE tempcuentaproveedor SET idproveedor=2608;
        ...
        IF FOUND AND (aux.nrobanco = 191  OR aux.nrobanco = 29 ) THEN --MaLaPi 12-10-2017 Cuando el banco es credicoop no es obligatorio el CUIL, como no siempre esta bien cargado no lo pongo
	...
	SELECT INTO laobsmodificada CONCAT('*#*MP',rminutaafil.nroordenpago,'-',rminutaafil.idcentroordenpago, '*#*OPC',unacuenta.nroopago);
...
ELSE 
	...
	SELECT  nroreintegro, anio,idcentroregional 
                ...
                WHERE idordenpagocontable =  unacuenta.idcomprobante
                     /* MaLaPi 09-12-2019 Comento porque si se anula una OPC y se vuelve a emitir otro, no encuentra el reintegro
	...
	and trim(split_part(trim(substring(opcobservacion from '%Pago Reintegro #"_________________#"%' for '#')), '-',3)) = idcentroregional */
        AND ( trim(split_part(trim(substring(opcobservacion from '%Pago Reintegro #"_________________#"%' for '#')), '-',1)) = nroreintegro OR trim(split_part(substring(opcobservacion from '%del reintegro #"____________#"%' for '#'),'-',1)) =nroreintegro  )
	and ( trim(split_part(trim(substring(opcobservacion from '%Pago Reintegro #"_________________#"%' for '#')), '-',2)) = anio OR trim(split_part(substring(opcobservacion from '%del reintegro #"____________#"%' for '#'),'-',2)) = anio )
	and ( trim(split_part(trim(substring(opcobservacion from '%Pago Reintegro #"____________#"%' for '#')), '-',3)) = idcentroregional  OR  trim(split_part(substring(opcobservacion from '%del reintegro #"____________#"%' for '#'),'-',3))::integer = idcentroregional )
...
/*
/*Dani comento 030723 se reemplaza la consulta original por la de abajo para que encuentre correctamente el beneficiario en casos donde paso de benef a titular*/
                         ...
                         CASE WHEN nullvalue(nrodoctitu) THEN concat(p.apellido,' ',p.nombres) ELSE 
                         concat(ptitu.apellido,' ',ptitu.nombres) END::text AS p_razonsocial
...
*/
                         SELECT INTO rcuentanrodoc
                         CASE WHEN (nullvalue(bs.nrodoctitu)or (not nullvalue(bb.nrodoc))) THEN p.nrodoc ELSE bs.nrodoctitu END AS nrodoc,
                         rpad('',11,' ')::varchar AS nrocuit
                        ,CASE WHEN (nullvalue(bs.nrodoctitu)or (not nullvalue(bb.nrodoc))) THEN p.tipodoc  ELSE bs.tipodoctitu END::integer  AS tipodoc,
                         CASE WHEN (nullvalue(bs.nrodoctitu)or (not nullvalue(bb.nrodoc))) THEN concat(p.apellido,' ',p.nombres) ELSE 
                         concat(ptitu.apellido,' ',ptitu.nombres) END::text AS p_razonsocial
         
                          FROM reintegrobenef AS rb  
                
                          LEFT JOIN persona AS p USING(nrodoc) --MaLapi 10-10-2017 En reintegrobenef esta solo la barra, y la barra puede cambiar en el 
                           LEFT JOIN beneficiariosborrados  AS bb on(bb.nrodoc=p.nrodoc and  bb.tipodoc=p.tipodoc)
		          LEFT JOIN benefsosunc AS bs on(bs.nrodoc=p.nrodoc  and    bs.tipodoc=p.tipodoc)
                          LEFT JOIN persona AS ptitu ON(bs.nrodoctitu=ptitu.nrodoc AND bs.tipodoctitu=ptitu.tipodoc)
