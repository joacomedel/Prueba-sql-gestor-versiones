CREATE OR REPLACE FUNCTION public.tesoreria_reportecontrolcajaconpendientes_contemporal(character varying)
...
GROUP BY ccfv.idcontrolcajafacturaventa,ccfv.centro,comprobante,elidcontrolcaja,idcontrolcaja,fv.tipofactura,idcentrocontrolcaja,elafiliado,comprobante,fv.fechaemision,nrocliente,cliente.barra, anulada, fvc.idvalorescaja,monto  
...
importerecibo as importe,idcontrolcajarecibo as idccc,ccr.idcentrocontrolcajarecibo idcentroccc,
 ...
 lower(replace(replace(replace(replace(replace(replace(descripcion,'.',''),'-','_'),')',''),'(',''),'/','_'),' ','')) as clave  
  ...
  WHILE  found LOOP
	vquery = concat('UPDATE  temp_tesoreria_reportecontrolcaja_contemporal SET col_',relem.idvalorescaja,' =',
        ...
        WHERE temp_tesoreria_reportecontrolcaja_contemporal.idccc = ', relem.idccc ,' and  temp_tesoreria_reportecontrolcaja_contemporal.idcentroccc=',relem.idcentroccc);
        EXECUTE vquery;
	FETCH cursorcc INTO relem;
   END LOOP;
   CLOSE cursorcc;
