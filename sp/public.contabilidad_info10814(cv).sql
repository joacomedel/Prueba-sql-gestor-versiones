CREATE OR REPLACE FUNCTION public.contabilidad_info10814(character varying)
	        ...
	        FROM (
  		       SELECT   afil.nrodoc,afil.barra  , afil.apellido, afil.nombres,
                    /*CASE WHEN not nullvalue ( dpa.iddeuda ) THEN  concat(dpa.iddeuda,'|',dpa.idcentrodeuda,'|',dpa.idpago,'|',dpa.idcentropago)
				     ...
				     ELSE '' END imputacion 
					 */
         	CASE WHEN not nullvalue ( dpa.iddeuda ) THEN  concat(dpa.iddeuda,'|',dpa.idcentrodeuda)
				     WHEN not nullvalue ( dpc.iddeuda ) THEN concat(dpc.iddeuda,'|',dpc.idcentrodeuda)
				     ELSE '' END imputacion 
			FROM facturaventa f
 ...
 */
       ...
       IF (rinfo.idasientogenericocomprobtipo =4 ) THEN /* 070923 Puede ser una minuta de imputacion */
       		SELECT INTO rinfoaux DISTINCT nrodoc,barra,  apellido, nombres ,  imputacion
  		FROM (
         		SELECT   CASE WHEN nullvalue(dpop_afil.nroordenpago) THEN  cli.nrodoc ELSE afil.nrodoc END as nrodoc, 
					...
					/*	 CASE WHEN not nullvalue(dpop_afil.iddeuda) THEN concat(dpop_afil.iddeuda,'|',dpop_afil.idcentrodeuda,'|',dpop_afil.idpago,'|',dpop_afil.idcentropago,' ')
							  ...
							  ELSE '' END imputacion
    */
						CASE WHEN not nullvalue(dpop_afil.iddeuda) THEN concat(dpop_afil.iddeuda,'|',dpop_afil.idcentrodeuda)
							  WHEN not nullvalue(dp_cli.iddeuda) THEN concat(dp_cli.iddeuda,'|',dp_cli.idcentrodeuda)
							  ELSE '' END imputacion	
				  FROM  ordenpago 
