CREATE OR REPLACE FUNCTION public.spparareporterecibocaja(character varying, character varying, character varying, character varying)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$/*
...
SELECT  unionordenes.nroorden, unionordenes.centro, unionordenes.fechaemision, unionordenes.tipo,
 ...
 CASE WHEN NOT NULLVALUE(reanulado) THEN 'Anulado' END AS anulado,
          ...
          r.idrecibo, 387 AS idconcepto/*, CASE WHEN nullvalue(ttnrocuentac.nrocuentac) THEN '40311'
...
and (elusuario=0 or recibousuario.idusuario= elusuario)
...
, importesrecibo.idformapagotipos,
 concat(formapagotipos.idformapagotipos, ' - ' , formapagotipos.fpabreviatura) AS tformapago
, recibo.idrecibo, 200 AS idconcepto--,cuentascontables.nrocuentac
...
CASE when nullvalue(ccp.idconcepto) then ccpna.idconcepto ELSE ccp.idconcepto END 
