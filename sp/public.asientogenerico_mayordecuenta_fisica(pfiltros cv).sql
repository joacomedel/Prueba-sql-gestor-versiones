CREATE OR REPLACE FUNCTION public.asientogenerico_mayordecuenta_fisica(pfiltros character varying)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
       rfiltros RECORD;
...
--asientogenerico_mayordecuenta_10321_2021
    CREATE  TABLE asientogenerico_mayordecuenta_fisico
	-- CREATE TABLE temp_asientogenerico_mayordecuenta_contemporal
     AS (
        select now() as emitido
        ,'1-ASIENTO#idasiento@2-COD CUENTA#codcuenta@3-CUENTA#cuenta@4-FECHA#fechacontable@5-DESCRIPCION#concepto@6-DEBE#debe@7-HABER#haber@8-SALDO#saldo<+>@9-IDComprobante#elidsiges@10-OBSERVACION#leyenda'::text as mapeocampocolumna
        from (
             SELECT 
                    rfiltros.fechaDesde,
                    rfiltros.fechaHasta,
...
--LIMIT 100
  ...
  OPEN  cmayor FOR
          SELECT * FROM asientogenerico_mayordecuenta_fisico order by fechacontable;
    FETCH cmayor INTO rmayor;
    WHILE FOUND LOOP
          UPDATE asientogenerico_mayordecuenta_fisico as t
  ...
  IF FOUND THEN  
         UPDATE asientogenerico_mayordecuenta_fisico 
--X MAIL DE ANDREA (ASUNTO DEUDORES POR ADHERENTES) SE MODIFICA
		 SET  leyenda = concat(t.leyenda,'Dif;',t.diferencia)
		 FROM (
                ...
                FROM asientogenerico_mayordecuenta_fisico
                group by  leyenda
        )as t
        WHERE  t.leyenda = asientogenerico_mayordecuenta_fisico.leyenda;  
  END IF;
