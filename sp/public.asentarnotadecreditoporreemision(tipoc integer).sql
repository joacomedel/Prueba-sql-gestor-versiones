CREATE OR REPLACE FUNCTION public.asentarnotadecreditoporreemision(tipoc integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
DECLARE
...
ordenesgeneradas Cursor for select * from ttordenesanuladas;
torden record;
itemv RECORD;
tdoc INTEGER;
...
VALUES(tipoc,elem.nrosucursal,elem.nrofactura,nrodocumento,tdoc,1000,torden.centro,torden.amuc,torden.efectivo,torden.debito,torden.credito,torden.cuentacorriente,torden.sosunc,current_date,formadepagocalculada,'NC');
...
IF (torden.tipo = 4) THEN
   ...
   VALUES(tipoc,elem.nrosucursal,'NC',elem.nrofactura,'50340',torden.cantordenes,sumaimporte,'Consultas',1);
ELSE
    IF (torden.tipo = 3) THEN
       /*Este es el caso de una orden de internacion*/
       INSERT INTO itemfacturaventa(tipocomprobante,nrosucursal,tipofactura,nrofactura,idconcepto,cantidad,importe,descripcion,idiva)
       VALUES(tipoc,elem.nrosucursal,'NC',elem.nrofactura,'50310',torden.cantordenes,sumaimporte,'Internación',1);
    ELSE
        ...
        WHILE FOUND LOOP
            ...
            ELSE
                INSERT INTO itemfacturaventa(tipocomprobante,nrosucursal,tipofactura,nrofactura,idconcepto,cantidad,importe,descripcion,idiva)
                VALUES(tipoc,elem.nrosucursal,'NC',elem.nrofactura,nrocuenta,cantidad1,importevalorizada,itemv.desccuenta,1);
                cantidad1 = itemv.cantidad;
                IF (tieneamuc) THEN
                   IF (itemv.porcentaje + 10 > 100) THEN
                      porcentajereal = itemv.porcentaje - (itemv.porcentaje + 10 -100);
        ...
        END LOOP;
        INSERT INTO itemfacturaventa(tipocomprobante,nrosucursal,tipofactura,nrofactura,idconcepto,cantidad,importe,descripcion,idiva)
        VALUES(tipoc,elem.nrosucursal,'NC',elem.nrofactura,nrocuenta,cantidad1,importevalorizada,descripcionactual,1);
        close items;
    END IF;
END IF;
 ...
 --  inserta las facturas en ttfacturasgeneradas
            INSERT INTO ttfacturasgeneradas (tipocomprobante, nrosucursal,tipofactura,nrofactura)
                VALUES (tipoc,elem.nrosucursal,'NC',elem.nrofactura);


INSERT INTO facturaorden(tipocomprobante,nrosucursal,tipofactura,nrofactura,nroorden,centro)
select tipoc, elem.nrosucursal,'NC', elem.nrofactura, ttordenesgeneradas.nroorden, ttordenesgeneradas.centro FROM ttordenesgeneradas;
