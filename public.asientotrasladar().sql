CREATE OR REPLACE FUNCTION public.asientotrasladar()
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
/****/
DECLARE
   	casientosrev CURSOR FOR SELECT *
                            ...
                            JOIN asientotrasladar using (nroasiento)
                            JOIN multivac.mapeocuentascontables as c ON (c.idcuentacontablemultivac = codcuentadestino)
                            WHERE --(nroasiento = 172902 or nroasiento = 157798 )and
                                   idmodulo = 3 and idejercicio=11
                                   and nullvalue(generado);
    rasientorev record;
    curitem refcursor;
    regitem record;
...
BEGIN
     ...
     WHILE found LOOP
                   ...
                   OPEN curitem for SELECT
                    ...
                    WHERE r.idasiento = rasientorev.idasiento AND r.idcuenta = rasientorev.codcuentaorigen ;

                    descriptconcepto = concatenar (concatenar(concatenar('REIMP - AS:',rasientorev.nroasiento::varchar), ': ' ) , rasientorev.concepto::varchar);
			        ...
			        xidasiento=currval('asientogenerico_idasientocontable_seq');
                   	...
                   	WHILE found LOOP
                          ...
                          INSERT INTO  asientogenericoitem(idasientogenerico,idcentroasientogenerico,acimonto,nrocuentac,acidescripcion,acid_h)
                          VALUES (xidasiento,centro(),abs(regitem.nuevomonto),rasientorev.nrocuentac,descriptconcepto,'D');

                    FETCH curitem INTO regitem;
			        END LOOP;
			        CLOSE curitem;
	 perform	cambiarestadoasientogenerico(xidasiento,centro(),1);
	 UPDATE asientotrasladar SET generado = now(),idasientogenerico = xidasiento WHERE idasientotrasladar = rasientorev.idasientotrasladar;
