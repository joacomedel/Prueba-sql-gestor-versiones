...
LEFT JOIN ( SELECT fmim.idmonodroga ,
	    ...
	    WHERE nrodoc =  CASE WHEN length($3)=7 THEN concat('0',$3::character varying) ELSE $3::character varying END 
		           -- VAS 040724   AND (nullvalue(fmimfechafin) OR 	fmimfechafin>=now() ) --- es una linea de auditoria vigente 
		            AND (safechaingreso <=$7 AND ($7<=fmimfechafin OR nullvalue(fmimfechafin))) -- VAS 040724  para que busque la cobertura en la fecha enviada por parametro
...
WHERE  mnroregistro = $1 --- info del medicamento
       ...
       AND (fechainivigencia <=$7 AND ($7<=fechafinvigencia OR nullvalue(fechafinvigencia))) -- VAS 040724  para que busque la cobertura en la fecha enviada por parametro
