CREATE OR REPLACE FUNCTION public.far_traercoberturasarticuloafiliado_2(character varying, bigint, bigint, bigint, integer, bigint, integer)
 RETURNS SETOF far_plancoberturamedicamentoafiliado
 LANGUAGE sql
AS $function$
/* $1 vmnroregistro - $2 vidafiliadoos - $3 vidafiliadososunc - $4 vidafiliadoamuc -$5 vidvalidacion
...
where (mnroregistro = $1  OR idarticulo = $1 ) AND idvalidacion = $5
...
JOIN adesfa_prepagas AS ap ON(avr.fincodigo=ap.idadesfa_prepagas)
JOIN far_obrasocial AS o USING(idobrasocial)
JOIN far_obrasocialmutual as osm ON osm.idobrasocial = o.idobrasocial AND osm.idmutual = 10
