CREATE OR REPLACE FUNCTION public.far_traerinfocoberturas_val_2()
...
IF NOT FOUND THEN
        ...
        IF FOUND THEN
              ...
              IF FOUND THEN  --Cargo en far_medicamento
                  insert into far_medicamento(idarticulo,idcentroarticulo,mnroregistro) values(rmed.idarticulo,rmed.idcentroarticulo,rarticulo.mnroregistro::integer);
                  vmnroregistro = rarticulo.mnroregistro; 
              ELSE 
...
-- Es Afiliado de SOSUNC
   ...
   IF NOT FOUND then  -- NO estÃ¡ cargado en far_afiliado para SOSUNC
      INSERT INTO far_afiliado(idobrasocial,aidafiliadoobrasocial,aapellidoynombre,iddireccion,nrocliente,barra,tipodoc,nrodoc)
...
for rcobinfomedi in SELECT idobrasocial,
  ...
  case when nullvalue(rarticulo.cantvendida) then vi.cantidadaprobada else rarticulo.cantvendida END as cantidadaprobada,
  idarticulo ,
  idcentroarticulo ,
  idrubro ,
  adescripcion ,
  ...
  porciva  FROM (select *
                 ...
                 (vmnroregistro,vidafiliadoos,vidafiliadososunc,vidafiliadoamuc,vidvalidacion,vidafiliadomutual,vquemutual)
                ) as coberturas
