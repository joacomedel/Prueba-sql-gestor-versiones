CREATE OR REPLACE FUNCTION public.contabilidad_balancecontable_asiento_resultado_cierre(pfiltros character varying)
...
BEGIN
---SELECT contabilidad_balancecontable_asiento_resultado_cierre('{idejerciciocontable=8}')
       salida = false;
	   diferencia = 0;
	   ...
	   ELSE 
			        ...
			        elconcepto = concat('Fecha al ',rejercicio.fecha_formato ,'. Concepto: Cierre de Cuentas Resultado ',rejercicio.anio,'.');
					...
					-- 4 busco los datos de las cuentas contables vinculadas al ejercicio
				 	...
				 	-- SOLO se deben tener en cuenta las cuentas que comienzan con 4/5
			
				 	OPEN cursor_sumasysaldo  FOR SELECT * FROM temp_contabilidad_balancecontable_contemporal 
				 	WHERE codcuenta ilike '4%'
				 ...
				 -- La diferencia debe incorporarse a la cuenta 30230 
