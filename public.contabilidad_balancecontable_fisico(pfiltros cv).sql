CREATE OR REPLACE FUNCTION public.contabilidad_balancecontable_fisico(pfiltros character varying)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
      rfiltros RECORD;
...
BEGIN
       ...
       IF not nullvalue(rejercicio.eccerrado) THEN 
				 ...
				 IF (diferencia > 0 ) THEN 
				 		INSERT INTO tempordenpagoimputacion (codigo, nrocuentac,debe ,haber) 
	             		VALUES (30240 ,'30240' , abs(diferencia),'0');
				 ELSE 
						INSERT INTO tempordenpagoimputacion (codigo, nrocuentac,debe ,haber) 
	             		VALUES (30240  ,'30240' ,'0',  abs(diferencia));
				 END IF;
