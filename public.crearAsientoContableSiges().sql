CREATE OR REPLACE FUNCTION public."crearAsientoContableSiges"()
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
    rliq RECORD;
...
BEGIN
	...
	where nullvalue(ltefechafin) and idtipoestadoliquidaciontarjeta=2 and idliquidacion=pidliquidacion;
		...
		values(pidliquidacion,2);
		    ...
		    where idestadoliquidaciontarjeta=xestado and idliquidacion=pidliquidacion and idtipoestadoliquidaciontarjeta<>2 and nullvalue(ltefechafin);
     	        end if;
          end;
        end if;
